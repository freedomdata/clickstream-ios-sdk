...
public final class ClickstreamSDK {
    ...
    /**
     Запрашивает конфигурацию флагов с backend для текущего устройства.
     - Parameters:
       - completion: Замыкание с результатом запроса.
     */
    public static func fetchFlags(completion: @escaping @Sendable (FetchFlagsResult) -> Void) {
        shared.fetchFlags(completion: completion)
    }

    /**
     Возвращает последнюю сохраненную конфигурацию флагов.
     - Returns: Локально сохраненная конфигурация флагов или nil.
     */
    public static func getFlags() -> [FlagInfo]? {
        shared.getFlags()
    }
    ...
    /**
     Запрашивает конфигурацию флагов через внутренний менеджер SDK.
     - Parameters:
       - completion: Замыкание с результатом запроса.
     */
    private func fetchFlags(completion: @escaping @Sendable (FetchFlagsResult) -> Void) {
        let manager: FlagManager? = queue.sync { _flagManager }
        guard let manager else {
            if let stored = FlagStorage().loadBestAvailable(deviceId: deviceId) {
                completion(.fallback(stored))
            } else {
                ClickstreamLogger.log("ClickstreamSDK is not initialized. Call initialize() before fetchFlags().")
                completion(.failure(.notInitialized))
            }
            return
        }

        manager.fetchFlags(completion: completion)
    }
     ...
     */
    private func getFlags() -> [FlagInfo]? {
        if let manager = queue.sync(execute: { _flagManager }) {
            return manager.getFlags()
        }
        return FlagStorage().loadBestAvailable(deviceId: deviceId)
    }
}