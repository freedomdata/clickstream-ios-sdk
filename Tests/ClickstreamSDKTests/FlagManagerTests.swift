...
final class FlagManagerTests: XCTestCase {
    ...
    func test_fetchFlags_onPersistenceFailure_retriesUntilSaved() {
        ...
        wait(for: [persistedExpectation], timeout: 5)

        XCTAssertEqual(storage.load(deviceId: deviceId), freshFlags)
    ...
    func test_fetchFlags_onSuccess_persistsToLocalCache() {
        ...
        XCTAssertEqual(
            ...
            "https://gateway.example.com/api/click-stream-rest-flag/flag?deviceId=\(deviceId)"
        )
        XCTAssertEqual(
