...
final class ClickstreamURLBuilderTests: XCTestCase {
    ...
    func test_flagsEndpointURL_withoutPath_usesSameHostAndPlatformPath() {
        ...
        XCTAssertEqual(
            url?.absoluteString,
            "https://gateway.example.com/api/click-stream-rest-flag/flag?deviceId=device-1"
        )
    }
    ...
    /**
     Проверяет безопасное кодирование deviceId в query-параметре.
     */
    func test_flagsEndpointURL_withSpecialCharacters_encodesDeviceIdAsQueryValue() {
        let url = ClickstreamURLBuilder.flagsEndpointURL(
            baseUrl: "https://gateway.example.com",
            deviceId: "device 1&group=beta"
        )

        XCTAssertEqual(
            url?.absoluteString,
            "https://gateway.example.com/api/click-stream-rest-flag/flag?deviceId=device%201%26group%3Dbeta"
        )
    }
}
